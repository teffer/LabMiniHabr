...
");
	...
	public static async Task AddRandomData(Db db, RandomDataGenerator rg) {
		...
		await InsertMultiple(cursor, "companies", companies);
		...
		await InsertMultiple(cursor, "app_users", users);
		...
		await InsertMultiple(cursor, "hubs", hubs);
		...
		await InsertMultiple(cursor, "articles", articles);
		await InsertLinks(cursor, rg, "article_hub_links", articles, "article_id", 0, 5, hubs, "hub_id");
		await InsertLinks(cursor, rg, "user_company_links", users, "user_id", 0, 2, companies, "company_id");
		...
		await InsertMultiple(cursor, "polls", polls);
		await InsertMultiple(cursor, "poll_variants", pollVariants);
		...
		await InsertMultiple(cursor, "article_comments", comments);
		var comment_bookmarks = new List<QueryParameters>();
		foreach (var comment in rg.NextArrayElements(comments, 0.1)) {
			foreach (var user in rg.NextArrayElements(users, 0.02)) {
				comment_bookmarks.Add(new QueryParameters() {
					{ "user_id", user.GetValue("id") },
					{ "comment_id", comment.GetValue("id") },
					{ "creation_time", rg.NextUtcDateTime() },
				});
			}
		}
		await InsertMultiple(cursor, "comment_bookmarks", comment_bookmarks);
		await cursor.Commit();
	}
	static async Task<int> InsertMultiple(DbCursor cursor, string tableName, IEnumerable<QueryParameters> multipleParameters, string conflictNames = "") {
